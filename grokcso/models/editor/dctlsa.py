...
class BasicLayer(nn.Module):
  def __init__(self, dim, resolution, embed_dim=50, depth=2, num_heads=8,
               window_size=8,
...
class LocalModule(nn.Sequential):
  def __init__(self, channels):
    ...
    self.add_module('pointwise_conv_0',
                    nn.Conv2d(channels, channels, kernel_size=1, bias=False))
    self.add_module('depthwise_conv',
                    nn.Conv2d(channels, channels, padding=1, kernel_size=3,
                              groups=channels, bias=False))
    self.add_module('pointwise_prenorm_1', nn.BatchNorm2d(channels))
    self.add_module('pointwise_conv_1',
                    nn.Conv2d(channels, channels, kernel_size=1, bias=False))
...
class WindowAttention(nn.Module):
  ...
  def forward(self, x, H, W, mask=None):
    ...
    qkv = self.qkv(local)
    # partition windows
    qkv = window_partition(qkv, self.window_size[
      0])  # nW*B, window_size, window_size, C

    B_, _, _, C = qkv.shape

    qkv = qkv.view(-1, self.window_size[0] * self.window_size[1],
                   C)  # nW*B, window_size*window_size, C

    N = self.window_size[0] * self.window_size[1]
    C = C // 3

    qkv = qkv.reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2,
                                                                             0,
                                                                             3,
                                                                             1,
                                                                             4)
...
def make_model(args, parent=False):
  model = DCTLSA(in_nc=args.n_colors, nf=args.channel,
                 num_modules=args.num_modules, out_nc=args.n_colors,
                 upscale=args.scale[0], num_head=args.num_head)
  return model


@MODELS.register_module()
class DCTLSA(BaseModel):
  def __init__(self,
               upscale=3,
               in_nc=1,
               nf=55,
  ...
  def forward(self, **kwargs) -> Union[Dict[str, torch.Tensor], list]:
    ...
    x_final = output.view(-1, 11*11*self.upscale*self.upscale)
