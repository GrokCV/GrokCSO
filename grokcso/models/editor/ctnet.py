...
class CTNET(BaseModel):
  ...
  def __init__(self,
    ...
    self.cfa1 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                          bias=False)

    self.cem2 = CEM(nf)
    self.ctb2 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa2 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                          bias=False)

    self.cem3 = CEM(nf)
    self.ctb3 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa3 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                          bias=False)

    self.cem4 = CEM(nf)
    self.ctb4 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa4 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                          bias=False)

    self.cem5 = CEM(nf)
    self.ctb5 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa5 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                          bias=False)

    self.cem6 = CEM(nf)
    self.ctb6 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa6 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                          bias=False)

    self.cem7 = CEM(nf)
    self.ctb7 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa7 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                          bias=False)

    self.cem8 = CEM(nf)
    self.ctb8 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa8 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                          bias=False)

    self.cem9 = CEM(nf)
    self.ctb9 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa9 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                          bias=False)

    self.cem10 = CEM(nf)
    self.ctb10 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa10 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                           bias=False)

    self.cem11 = CEM(nf)
    self.ctb11 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa11 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                           bias=False)

    self.cem12 = CEM(nf)
    self.ctb12 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa12 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                           bias=False)

    self.cem13 = CEM(nf)
    self.ctb13 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa13 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                           bias=False)

    self.cem14 = CEM(nf)
    self.ctb14 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa14 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                           bias=False)

    self.cem15 = CEM(nf)
    self.ctb15 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa15 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                           bias=False)

    self.cem16 = CEM(nf)
    self.ctb16 = CTB(in_chs=nf, mid_chs=nf // 2, out_chs=nf)
    self.cfa16 = nn.Conv2d(nf, nf // nb, 3, 1, 3 // 2, groups=nf // nb,
                           bias=False)
  ...
  def forward(self, **kwargs):
    ...
    ILR = F.interpolate(x, scale_factor=self.scale, mode="bilinear",
                        align_corners=False)
    out = out + ILR
...
class CFE(nn.Module):
  def __init__(self, inplanes, planes, stride, padding, dilation, groups,
    ...
    self.k1 = nn.Sequential(
      nn.Conv2d(
        inplanes, planes, kernel_size=1, stride=1, padding=0, dilation=dilation,
    ...
    self.k4 = nn.Sequential(
      nn.Conv2d(
        planes, planes, kernel_size=1, stride=stride, padding=0,
...
class SqueezeExcite(nn.Module):
  def __init__(
    self, in_chs, se_ratio=0.25, reduced_base_chs=None, act_layer=nn.ReLU,
...
class CTL(nn.Module):
  def __init__(self, inp, oup, kernel_size=1, ratio=2, dw_size=3, stride=1,
    ...
    self.cfe = nn.Sequential(
      CFE(inplanes=inp, planes=init_channels, stride=stride, padding=1,
          dilation=1, groups=1, pooling_r=2),
